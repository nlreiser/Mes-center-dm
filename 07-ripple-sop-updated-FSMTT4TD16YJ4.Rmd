...
<br/>
    ...
    c. To search for a group of potential participants that all meet the same criteria, you can filter the registry. 
        i. The below example search is for the number of people that were added to the Ripple registry from 1/1/23 until the date the report was filtered, are above 80 years old, and have not already been added to the SuperAging study. This resulted in 14 participants.
```{r, echo=FALSE, fig.cap="", out.width = '100%', out.extra='style="background-color: #41444B; padding:2px;"', fig.align = 'center'}
...
### Recruitment
1. The Recruitment tab is where sources and strategies may be updated for recruitment tracking purposes. This page is specific to one study, and recruitment sources need to be tracked center wide, so study specific recruitment sources are not used for any Mesulam Center studies. 
...
### Study Settings 
1. The Study Settings tab is where things such as display, statuses, or other features may be edited. Study Coordinators and Research assistants should not make any changes or edits to the Study Settings. Requests for changes should be shared with the study manager.
...
3. To access a participant’s record, click the blue arrow next to their entry (in either potential participants or enrolled participants in any study or in the registry). Ideally, the record should be as complete as possible. It should be updated as soon as possible when any changes are available.
4. There are 9 sections to the record:
...
1. This section has the participant’s first and last name at the top. You can change the first and last name by clicking on the name. 2. Custom ID and family ID
    a. The custom ID is used as the specific study ID, and the family ID is used to connect family members that each have their own ripple records.
3. Global ID
    a. The global ID is assigned to all individuals at the time that they are added to Ripple. It is a 15-17 digit alpha-numeric unique identifier.
...
1. Comments
    ...
    b. Comments should not be used to log contact attempts (see below). If a comment needs to be logged as a result of a contact attempt, log the contact attempt, then log the comment. 
    d. All comments are automatically logged as global, meaning they are visible to all studies. If a comment needs to be available to only the current study, unselect the “global” box. 
...
## Adding a Participant from an Event to Outreach Events
1. Search the participant's last name in the Registry.
2. If the participant is already in Ripple:
    a. Add them to the “Outreach Events” page.  
...
4. Send out the registry survey link to everyone that has not already completed the registry: 
    a. All participants will be automatically added to the status of “Registry Link not Sent” (at the top of the Potential Participants page) 
    b. Check the box next to the first participant on the list for the “Registry Link not Sent” category.  
    c. At the top of this section, check the “Select All in Category” check box.  
    d. In the drop down menu, select "Send Registry Link Email (Not enough info).
    e. Click "Apply" 
    f. Select the appropriate email, either general, specific to clinical trials, or specific to super aging. They are all almost identical.  
    g. Click create email 
    h. Once open, add event name and event date in the body of the email.  
    i. In the “mark email as” section, mark the email as a contact and global.  
    j. Click “send emails” 
5. Move all participants that have been sent an email to the status of “Registry Survey Sent via Email” 
6. Every Friday, participants will be moved out of the status of “Registry Survey Sent via Email” to a category that represents where they are in the recruitment process. 
